...
use crate::access_control::{add_verifier_role, is_verifier, remove_verifier_role, require_verifier};
...
pub enum BondTier { Bronze, Silver, Gold, Platinum }
...
pub enum DataKey {
    ...
    GovernanceGovernors, GovernanceQuorumBps, GovernanceMinGovernors,
    FeeTreasury, FeeBps,
    EvidenceCounter,
    Evidence(u64),
    ...
    Paused,
    PauseSigner(Address),
    PauseSignerCount, PauseThreshold, PauseProposalCounter,
    PauseProposal(u64),
...
impl CredenceBond {
    fn acquire_lock(e: &Env) {
        if Self::check_lock(e) { panic!("reentrancy detected"); }
    ...
    fn check_lock(e: &Env) -> bool {
        e.storage().instance().get(&Self::lock_key(e)).unwrap_or(false)
    }
    fn lock_key(e: &Env) -> Symbol { Symbol::new(e, "lock") }
    fn callback_key(e: &Env) -> Symbol { Symbol::new(e, "callback") }
    #[allow(dead_code)]
    ...
    pub fn set_emergency_config(
        e: Env, admin: Address, governance: Address,
        treasury: Address, emergency_fee_bps: u32, enabled: bool,
    ) {
        admin.require_auth();
        Self::require_admin_internal(&e, &admin);
    ...
    pub fn set_emergency_mode(e: Env, admin: Address, governance: Address, enabled: bool) {
        ...
        if governance != cfg.governance { panic!("not governance"); }
        admin.require_auth();
    ...
    pub fn emergency_withdraw(
        e: Env, admin: Address, governance: Address, amount: i128, reason: Symbol,
    ) -> IdentityBond {
        Self::require_admin_internal(&e, &admin);
        let cfg = emergency::get_config(&e);
        if governance != cfg.governance { panic!("not governance"); }
        if !cfg.enabled { panic!("emergency mode disabled"); }
        if amount <= 0 { panic!("amount must be positive"); }
        admin.require_auth();
        governance.require_auth();
        let key = DataKey::Bond;
        let mut bond: IdentityBond = e.storage().instance().get(&key)
            .unwrap_or_else(|| panic!("no bond"));
        let available = bond.bonded_amount.checked_sub(bond.slashed_amount)
            .expect("slashed exceeds bonded");
        if amount > available { panic!("insufficient balance for withdrawal"); }
        ...
        if bond.slashed_amount > bond.bonded_amount { panic!("slashed exceeds bonded"); }
        let new_tier = tiered_bond::get_tier_for_amount(bond.bonded_amount);
        tiered_bond::emit_tier_change_if_needed(&e, &bond.identity, old_tier, new_tier);
        let record_id = emergency::store_record(
            &e, bond.identity.clone(), amount, fee_amount, net_amount,
            cfg.treasury.clone(), admin, governance, reason.clone(),
        );
        emergency::emit_emergency_withdrawal_event(
            &e, record_id, &bond.identity, amount, fee_amount, net_amount, &reason,
        );
    ...
    pub fn register_attester(e: Env, attester: Address) {
        ...
        e.events().publish((Symbol::new(&e, "attester_registered"),), attester);
    }
    ...
    pub fn unregister_attester(e: Env, attester: Address) {
        ...
        e.events().publish((Symbol::new(&e, "attester_unregistered"),), attester);
    }

    pub fn is_attester(e: Env, attester: Address) -> bool { is_verifier(&e, &attester) }
    ...
    pub fn get_verifier_stake_requirement(e: Env) -> i128 { verifier::get_min_stake(&e) }

    pub fn register_verifier(
        e: Env, verifier_addr: Address, stake_deposit: i128,
    ...
    pub fn deactivate_verifier_by_admin(
        e: Env, admin: Address, verifier_addr: Address,
    ) -> verifier::VerifierInfo {
        admin.require_auth();
        Self::require_admin_internal(&e, &admin);
    ...
    pub fn withdraw_verifier_stake(
        e: Env, verifier_addr: Address, amount: i128,
    ) -> verifier::VerifierInfo {
        verifier_addr.require_auth();
    ...
    pub fn set_verifier_reputation(
        e: Env, admin: Address, verifier_addr: Address, new_reputation: i128,
    ) {
        admin.require_auth();
        Self::require_admin_internal(&e, &admin);
    ...
    pub fn get_usdc_token(e: Env) -> Address { token_integration::get_token(&e) }
    pub fn get_usdc_network(e: Env) -> Option<String> { token_integration::get_usdc_network(&e) }
    ...
    pub fn create_bond_with_rolling(
        e: Env, identity: Address, amount: i128, duration: u64,
        is_rolling: bool, notice_period_duration: u64,
    ) -> IdentityBond {
        pausable::require_not_paused(&e);
        ...
        let bond = IdentityBond {
            identity: identity.clone(), bonded_amount: net_amount, bond_start,
            bond_duration: duration, slashed_amount: 0, active: true,
            is_rolling, withdrawal_requested_at: 0, notice_period_duration,
        };
    ...
    pub fn get_identity_state(e: Env) -> IdentityBond {
        e.storage().instance().get::<_, IdentityBond>(&DataKey::Bond)
            .unwrap_or_else(|| panic!("no bond"))
    }
    ...
    pub fn add_attestation(
        ...
        let is_authorized: bool = e.storage().instance()
            .get(&DataKey::Attester(attester.clone())).unwrap_or(false);
        if !is_authorized { panic!("unauthorized attester"); }
        let dup_key = DataKey::DuplicateCheck(attester.clone(), subject.clone(), attestation_data.clone());
        if e.storage().instance().has(&dup_key) { panic!("duplicate attestation"); }
        ...
        let mut attestations: Vec<u64> = e.storage().instance()
            .get(&subject_key).unwrap_or(Vec::new(&e));
    ...
    pub fn revoke_attestation(
        ...
        if attestation.verifier != attester { panic!("only original attester can revoke"); }
        if attestation.revoked { panic!("attestation already revoked"); }
        attestation.revoked = true;
        e.storage().instance().set(&key, &attestation);
        verifier::record_attestation_revoked(&e, &attester, attestation.weight);
        e.events().publish(
            (Symbol::new(&e, "attestation_revoked"), attestation.identity.clone()),
            (attestation_id, attester),
        );
    }

    pub fn get_attestation(e: Env, attestation_id: u64) -> Attestation {
        e.storage().instance().get(&DataKey::Attestation(attestation_id))
            .unwrap_or_else(|| panic!("attestation not found"))
    }
    pub fn get_subject_attestations(e: Env, subject: Address) -> Vec<u64> {
        e.storage().instance().get(&DataKey::SubjectAttestations(subject))
            .unwrap_or(Vec::new(&e))
    }
    pub fn get_subject_attestation_count(e: Env, subject: Address) -> u32 {
        e.storage().instance().get(&DataKey::SubjectAttestationCount(subject)).unwrap_or(0)
    }
    ...
    pub fn withdraw(e: Env, amount: i128) -> IdentityBond { Self::withdraw_bond(e, amount) }

    pub fn withdraw_bond(e: Env, amount: i128) -> IdentityBond {
        let key = DataKey::Bond;
        let mut bond = e.storage().instance().get::<_, IdentityBond>(&key)
            .unwrap_or_else(|| panic!("no bond"));
        ...
        if bond.is_rolling {
            ...
            if !rolling_bond::can_withdraw_after_notice(now, bond.withdrawal_requested_at, bond.notice_period_duration) {
                panic!("cooldown window not elapsed; request_withdrawal first");
            }
        } else if now < end {
        ...
        let old_tier = tiered_bond::get_tier_for_amount(bond.bonded_amount);
        bond.bonded_amount = bond.bonded_amount.checked_sub(amount).expect("underflow");
        if bond.slashed_amount > bond.bonded_amount { bond.slashed_amount = bond.bonded_amount; }
        let new_tier = tiered_bond::get_tier_for_amount(bond.bonded_amount);
        tiered_bond::emit_tier_change_if_needed(&e, &bond.identity, old_tier, new_tier);
        e.storage().instance().set(&key, &bond);
    ...
    pub fn withdraw_early(e: Env, amount: i128) -> IdentityBond {
        ...
        let now = e.ledger().timestamp();
        let end = bond.bond_start.saturating_add(bond.bond_duration);
        if now >= end { panic!("use withdraw for post lock-up"); }
        let available = bond.bonded_amount.checked_sub(bond.slashed_amount)
            .expect("slashed exceeds bonded");
        if amount > available { panic!("insufficient balance for withdrawal"); }
        let (treasury, penalty_bps) = early_exit_penalty::get_config(&e);
        let remaining = end.saturating_sub(now);
        let penalty = early_exit_penalty::calculate_penalty(amount, remaining, bond.bond_duration, penalty_bps);
        ...
        if penalty > 0 { token_integration::transfer_from_contract(&e, &treasury, penalty); }
        let old_tier = tiered_bond::get_tier_for_amount(bond.bonded_amount);
        bond.bonded_amount = bond.bonded_amount.checked_sub(amount).expect("underflow");
        if bond.slashed_amount > bond.bonded_amount { panic!("slashed exceeds bonded"); }
        let new_tier = tiered_bond::get_tier_for_amount(bond.bonded_amount);
    ...
    pub fn request_withdrawal(e: Env) -> IdentityBond {
        ...
        let mut bond: IdentityBond = e.storage().instance().get(&key)
            .unwrap_or_else(|| panic!("no bond"));
        bond.identity.require_auth();
        if !bond.is_rolling { panic!("not a rolling bond"); }
        if bond.withdrawal_requested_at != 0 { panic!("withdrawal already requested"); }
    ...
    pub fn renew_if_rolling(e: Env) -> IdentityBond {
        ...
        let mut bond: IdentityBond = e.storage().instance().get(&key)
            .unwrap_or_else(|| panic!("no bond"));
        if !bond.is_rolling { return bond; }
        let now = e.ledger().timestamp();
        if !rolling_bond::is_period_ended(now, bond.bond_start, bond.bond_duration) { return bond; }
    ...
    pub fn slash(e: Env, admin: Address, amount: i128) -> IdentityBond {
        admin.require_auth();
        Self::require_admin_internal(&e, &admin);
        if amount < 0 { panic!("slash amount must be non-negative"); }
        slashing::slash_bond(&e, &admin, amount)
    }

    pub fn initialize_governance(e: Env, admin: Address, governors: Vec<Address>, quorum_bps: u32, min_governors: u32) {
        pausable::require_not_paused(&e);
        Self::require_admin_internal(&e, &admin);
    ...
    pub fn propose_slash(e: Env, proposer: Address, amount: i128) -> u64 {
        ...
        if proposer != admin && !is_governor { panic!("not admin or governor"); }
    ...
    pub fn execute_slash_with_governance(e: Env, proposer: Address, proposal_id: u64) -> IdentityBond {
        pausable::require_not_paused(&e);
        proposer.require_auth();
        let proposal = governance_approval::get_proposal(&e, proposal_id)
            .unwrap_or_else(|| panic!("proposal not found"));
        if proposal.proposed_by != proposer { panic!("only proposer can execute"); }
        let executed = governance_approval::execute_slash_if_approved(&e, proposal_id);
        if !executed { panic!("proposal not approved"); }
    ...
    pub fn get_fee_config(e: Env) -> (Option<Address>, u32) { fees::get_config(&e) }
    ...
    pub fn set_callback(e: Env, callback: Address) {
        e.storage().instance().set(&Self::callback_key(&e), &callback);
    }
    ...
    pub fn is_locked(e: Env) -> bool { Self::check_lock(&e) }

    pub fn get_slash_proposal(e: Env, proposal_id: u64) -> Option<governance_approval::SlashProposal> {
    ...
    pub fn get_governors(e: Env) -> Vec<Address> { governance_approval::get_governors(&e) }
    pub fn get_governance_delegate(e: Env, governor: Address) -> Option<Address> {
        governance_approval::get_delegate(&e, &governor)
    }
    pub fn get_quorum_config(e: Env) -> (u32, u32) { governance_approval::get_quorum_config(&e) }
    ...
    pub fn top_up(e: Env, amount: i128) -> IdentityBond {
        ...
        if amount <= 0 { panic!("amount must be positive"); }
        bond.identity.require_auth();
        let new_bonded = bond.bonded_amount.checked_add(amount).expect("top-up overflow");
    ...
    pub fn increase_bond(e: Env, caller: Address, amount: i128) -> IdentityBond {
        ...
        Self::with_reentrancy_guard(&e, || {
            ...
            let mut bond = e.storage().instance().get::<_, IdentityBond>(&key)
                .unwrap_or_else(|| panic!("no bond"));
            if bond.identity != caller { panic!("not bond owner"); }
            let token_addr: Address = e.storage().instance().get(&DataKey::BondToken)
                .unwrap_or_else(|| panic!("bond token not configured"));
            let old_amount = bond.bonded_amount;
            let new_amount = old_amount.checked_add(amount).expect("bond increase overflow");
    ...
    pub fn extend_duration(e: Env, additional_duration: u64) -> IdentityBond {
        ...
        let _end = bond.bond_start.checked_add(bond.bond_duration)
    ...
    pub fn submit_evidence(
        e: Env, submitter: Address, proposal_id: u64,
        hash: String, hash_type: EvidenceType, description: Option<String>,
    ) -> u64 {
    ...
    pub fn get_evidence_count(e: Env) -> u64 { evidence::get_evidence_count(&e) }
    ...
    pub fn withdraw_bond_full(e: Env, identity: Address) -> i128 {
        ...
        let bond: IdentityBond = e.storage().instance().get(&bond_key)
            .unwrap_or_else(|| panic!("no bond"));
        if bond.identity != identity { Self::release_lock(&e); panic!("not bond owner"); }
        if !bond.active { Self::release_lock(&e); panic!("bond not active"); }
    ...
    pub fn slash_bond(e: Env, admin: Address, slash_amount: i128) -> i128 {
        admin.require_auth();
        if slash_amount < 0 { panic!("slash amount must be non-negative"); }
        Self::acquire_lock(&e);
        let stored_admin: Address = e.storage().instance().get(&DataKey::Admin)
            .unwrap_or_else(|| panic!("no admin"));
        if stored_admin != admin { Self::release_lock(&e); panic!("not admin"); }
        let bond_key = DataKey::Bond;
        let bond: IdentityBond = e.storage().instance().get(&bond_key)
            .unwrap_or_else(|| panic!("no bond"));
        if !bond.active { Self::release_lock(&e); panic!("bond not active"); }
        let new_slashed = bond.slashed_amount.checked_add(slash_amount)
            .expect("slashing overflow");
        if new_slashed > bond.bonded_amount { Self::release_lock(&e); panic!("slash exceeds bond"); }
        let updated = IdentityBond {
    ...
    pub fn get_cooldown_period(e: Env) -> u64 { cooldown::get_cooldown_period(&e) }

    pub fn request_cooldown_withdrawal(e: Env, requester: Address, amount: i128) -> CooldownRequest {
        requester.require_auth();
        if amount <= 0 { panic!("amount must be positive"); }
        let bond = e.storage().instance().get::<_, IdentityBond>(&DataKey::Bond)
            .unwrap_or_else(|| panic!("no bond"));
        if bond.identity != requester { panic!("requester is not the bond holder"); }
        let available = bond.bonded_amount.checked_sub(bond.slashed_amount)
            .expect("slashed exceeds bonded");
        if amount > available { panic!("amount exceeds available balance"); }
        let req_key = DataKey::CooldownReq(requester.clone());
        if e.storage().instance().has(&req_key) { panic!("cooldown request already pending"); }
        let request = CooldownRequest {
            requester: requester.clone(), amount,
    ...
    pub fn execute_cooldown_withdrawal(e: Env, requester: Address) -> IdentityBond {
        requester.require_auth();
        let req_key = DataKey::CooldownReq(requester.clone());
        let request: CooldownRequest = e.storage().instance().get(&req_key)
        ...
        let bond_key = DataKey::Bond;
        let mut bond = e.storage().instance().get::<_, IdentityBond>(&bond_key)
            .unwrap_or_else(|| panic!("no bond"));
        let available = bond.bonded_amount.checked_sub(bond.slashed_amount)
            .expect("slashed exceeds bonded");
        if request.amount > available { panic!("insufficient balance for withdrawal"); }
        bond.bonded_amount = bond.bonded_amount.checked_sub(request.amount).expect("underflow");
        if bond.slashed_amount > bond.bonded_amount { panic!("slashed exceeds bonded after withdrawal"); }
    ...
    pub fn cancel_cooldown(e: Env, requester: Address) {
        requester.require_auth();
        let req_key = DataKey::CooldownReq(requester.clone());
        if !e.storage().instance().has(&req_key) { panic!("no cooldown request to cancel"); }
        e.storage().instance().remove(&req_key);
        cooldown::emit_cooldown_cancelled(&e, &requester);
    }

    pub fn get_cooldown_request(e: Env, requester: Address) -> CooldownRequest {
        e.storage().instance().get(&DataKey::CooldownReq(requester))
            .unwrap_or_else(|| panic!("no cooldown request"))
    }
}
...
#[contractimpl]
impl CredenceBond {
    pub fn is_paused(e: Env) -> bool { pausable::is_paused(&e) }
    pub fn pause(e: Env, caller: Address) -> Option<u64> { pausable::pause(&e, &caller) }
    pub fn unpause(e: Env, caller: Address) -> Option<u64> { pausable::unpause(&e, &caller) }
...
#[cfg(test)] mod test;
#[cfg(test)] mod test_reentrancy;
#[cfg(test)] mod test_attestation;
#[cfg(test)] mod test_batch;
#[cfg(test)] mod test_attestation_types;
#[cfg(test)] mod test_validation;
#[cfg(test)] mod test_governance_approval;
#[cfg(test)] mod test_parameters;
#[cfg(test)] mod test_fees;
#[cfg(test)] mod integration;
#[cfg(test)] mod test_increase_bond;
#[cfg(test)] mod security;
#[cfg(test)] mod test_pausable;
#[cfg(test)] mod fuzz;
#[cfg(test)] mod test_duration_validation;
#[cfg(test)] mod test_access_control;
#[cfg(test)] mod test_cooldown;
#[cfg(test)] mod test_events;
#[cfg(test)] mod test_early_exit_penalty;
#[cfg(test)] mod test_verifier;
#[cfg(test)] mod test_evidence;
#[cfg(test)] mod test_emergency;
#[cfg(test)] mod test_rolling_bond;
#[cfg(test)] mod test_tiered_bond;
#[cfg(test)] mod test_slashing;
#[cfg(test)] mod test_withdraw_bond;
#[cfg(test)] mod test_math;
#[cfg(test)] mod token_integration_test;
#[cfg(test)] mod test_replay_prevention;
#[cfg(test)] mod test_weighted_attestation;
